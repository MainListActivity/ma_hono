...
DATABASE_ID=$(wrangler d1 list --json 2>/dev/null | jq -r ".[] | select(.name==\"$D1_NAME\") | .uuid // empty")
if [ -z "$DATABASE_ID" ]; then
  echo "[D1] 不存在，创建中..."
  wrangler d1 create "$D1_NAME" > /dev/null
  DATABASE_ID=$(wrangler d1 list --json | jq -r ".[] | select(.name==\"$D1_NAME\") | .uuid")
fi
...
for KV_NAME in "${KV_NAMES[@]}"; do
  ...
  echo "[KV] 查找或创建 $TITLE ..."
  KV_ID=$(wrangler kv namespace list --json 2>/dev/null | jq -r ".[] | select(.title==\"$TITLE\") | .id // empty")
  if [ -z "$KV_ID" ]; then
    echo "[KV] 不存在，创建中..."
    wrangler kv namespace create "$KV_NAME" > /dev/null
    KV_ID=$(wrangler kv namespace list --json | jq -r ".[] | select(.title==\"$TITLE\") | .id")
  fi
...
R2_EXISTS=$(wrangler r2 bucket list --json 2>/dev/null | jq -r ".[] | select(.name==\"$R2_BUCKET\") | .name // empty")
if [ -z "$R2_EXISTS" ]; then
  echo "[R2] 不存在，创建中..."
