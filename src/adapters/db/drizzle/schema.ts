...
export const clientAuthMethodPolicies = sqliteTable(
  "client_auth_method_policies",
  {
    ...
    tenantId: text("tenant_id")
      .notNull()
      .references(() => tenants.id, { onDelete: "cascade" }),
    passwordEnabled: integer("password_enabled", { mode: "boolean" }).notNull().default(0),
    passwordAllowRegistration: integer("password_allow_registration", { mode: "boolean" }).notNull().default(0),
    magicLinkEnabled: integer("magic_link_enabled", { mode: "boolean" }).notNull().default(0),
    magicLinkAllowRegistration: integer("magic_link_allow_registration", { mode: "boolean" }).notNull().default(0),
    passkeyEnabled: integer("passkey_enabled", { mode: "boolean" }).notNull().default(0),
    passkeyAllowRegistration: integer("passkey_allow_registration", { mode: "boolean" }).notNull().default(0),
    googleEnabled: integer("google_enabled", { mode: "boolean" }).notNull().default(0),
    appleEnabled: integer("apple_enabled", { mode: "boolean" }).notNull().default(0),
    facebookEnabled: integer("facebook_enabled", { mode: "boolean" }).notNull().default(0),
    wechatEnabled: integer("wechat_enabled", { mode: "boolean" }).notNull().default(0),
    createdAt: text("created_at").notNull(),
    updatedAt: text("updated_at").notNull()
  },
  (table) => ({
